...
{
    if([GlobalAudioPlayer sharePlayer].gAudioPlayer != nil){
        [[GlobalAudioPlayer sharePlayer].gAudioPlayer stop];
        [GlobalAudioPlayer sharePlayer].gAudioPlayer = nil;
    }
    [_audioPlayer stop];
    _audioPlayer = nil;
...
- (void)setAudioDataWithUrl:(NSURL *)audioURL
{
    ...
    _audioData = [NSData dataWithContentsOfFile:audioURL];
...
- (void)updateProgressTimer:(NSTimer *)sender
{
    if (self.audioPlayer.playing) {
        ...
        NSLog(@"currentTime >>>>>>>>>>>>%d >>>>>>>>>>>>>>%d",self.audioPlayer.currentTime,self.audioPlayer.duration);
    }
}
...
- (void)onPlayButton:(UIButton *)sender
{
    ...
    if ([GlobalAudioPlayer sharePlayer].gAudioPlayer != nil && [GlobalAudioPlayer sharePlayer].gAudioPlayer.playing) {
        ...
        [UIView transitionWithView:self.playButton
                        ...
                        completion:nil];
        [GlobalAudioPlayer sharePlayer].gAudioPlayer = self.audioPlayer;
...
- (UIView *)mediaView
{
    if (self.audioData && self.cachedMediaView == nil) {
        ...
        playView.backgroundColor = [UIColor colorWithRed:238.0f/255.0f green:245.0f/255.0f blue:255.0f/255.0f alpha:1.0];
