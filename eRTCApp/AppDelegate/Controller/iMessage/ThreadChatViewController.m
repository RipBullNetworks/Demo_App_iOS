...
@end
@interface ThreadChatViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIViewControllerPreviewingDelegate,MPMediaPickerControllerDelegate, JSQAudioRecorderViewDelegate,CLLocationManagerDelegate,CNContactViewControllerDelegate,CNContactPickerDelegate, UIGestureRecognizerDelegate,GiphyDelegate,UIDocumentPickerDelegate,UIDocumentInteractionControllerDelegate, UITableViewDelegate, UITableViewDataSource, ChatReactionsDelegateDelegate, EmojisViewControllerDelegate, ChatReplyCountDelegate, JSQAudioMediaItemDelegate, AudioClickable, ChatUndoMsgDelegate> {
...
@implementation ThreadChatViewController
...
- (void)didupdateStatus:(NSNotification *) notification{
    ...
    if (dictData[@"eventData"] != nil && dictData[@"eventData"] != [NSNull null]) {
        ...
        NSString *ertcUser = eventData[@"targetUser"][App_User_ID];
        if ([ertcUser isEqualToString:self->_dictUserDetails[App_User_ID]]) {
...
-(void)showUserBlock:(BOOL *)blockUnBlock {
    if (blockUnBlock) {
        ...
        UILabel *lblTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 110)];
...
- (void)getChatHistory {
    if (self.strThreadId != nil ) {
        ...
        if (![Helper stringIsNilOrEmpty:[self.dictThreadMsgDetails valueForKey:MsgUniqueId]]) {
        [[eRTCCoreDataManager sharedInstance] getUserReplyThreadChatHistoryWithThreadID:self.strThreadId withParentID:[self.dictThreadMsgDetails valueForKey:MsgUniqueId] andCompletionHandler:^(id ary, NSError *err) {
...
//    [self.inputToolbar.viewSendToChannel setUserInteractionEnabled:YES];
//    [self.inputToolbar showSendToChannelView:YES];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info {
    [self dismissViewControllerAnimated:YES completion:^{
        ...
        NSLog(@"ThreadChatViewController ->  imagePickerController -> mediaType -> %@",mediaType);
        ...
        else if ( [mediaType isEqualToString:@"public.audio"])
        {
            NSLog(@"Picked a vedio  URL %@",  [info objectForKey:UIImagePickerControllerMediaURL]);
            NSURL *url =  [info objectForKey:UIImagePickerControllerMediaURL];
            NSLog(@"ThreadChatViewController ->  imagePickerController -> audio -> %@",[url absoluteString]);
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        NSString *erctId = [NSString stringWithFormat:@"%@", [self.dictThreadMsgDetails valueForKey:SendereRTCUserId]];
        
        NSString *epochTime = [NSString stringWithFormat:@"%@", [self.dictThreadMsgDetails valueForKey:@"createdAt"]];
        ...
        [[eRTCChatManager sharedChatInstance] getuserInfoWithERTCId:erctId andCompletion:^(id  _Nonnull json, NSString * _Nonnull errMsg) {
            ...
            if (arrUser.count > 0) {
                ...
                if (dictUser[User_Name] != nil && dictUser[User_Name] != [NSNull null]) {
                    headerView.lblName.text = [NSString stringWithFormat:@"%@",dictUser[User_Name]];
                }else{
                    headerView.lblName.text = [NSString stringWithFormat:@"%@",_dictThreadMsgDetails[User_Name]];
                }
        ...
        if ([self.dictThreadMsgDetails valueForKey:@"message"] != nil &&  [self.dictThreadMsgDetails valueForKey:@"message"] != [NSNull null]) {
            if ( [[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"text"]) {
                NSString *removedTags = [Helper getRemoveMentionTags:  [self.dictThreadMsgDetails valueForKey:@"message"]];
                NSString *strMessage = @"";
                if (editTags != nil) {
                    ...
                    if (_dictThreadMsgDetails != NULL && ![_dictThreadMsgDetails[IsDeletedMSG] boolValue] && [_dictThreadMsgDetails[IsEdited] isEqual:@1]){
                        NSString *_message = removedTags;
                        strMessage = [NSString stringWithFormat:@"%@%@",_message, EditedString];
                        NSMutableAttributedString *attrString = [Helper mentionHighlightedAttributedStringByNames:_userNames message:strMessage].mutableCopy;
                        NSString *orignalMessage = [attrString.string stringByReplacingOccurrencesOfString:EditedString withString:@""];
                ...
                headerView.lblMessage.text = [[NSString stringWithFormat:@"%@", [self.dictThreadMsgDetails valueForKey:@"msgType"]] capitalizedString];
            }
        } else {
            headerView.lblMessage.text = [[NSString stringWithFormat:@"%@", [self.dictThreadMsgDetails valueForKey:@"msgType"]] capitalizedString];
        }
        ...
        if ([self.dictThreadMsgDetails valueForKey:@"isFavourite"] != nil && [self.dictThreadMsgDetails valueForKey:@"isFavourite"] != [NSNull null]) {
            if ([[self.dictThreadMsgDetails valueForKey:@"isFavourite"] intValue] == 0) {
                // Not Fav
...
-(NSString*)getSeenMsgStatusIndexFromThreadChat:(NSInteger)indexItem{
    ...
    if ((isLastMessage && isSelf) || isSending) {
        ...
        //if (currentMessage.msgStatus != nil){
            if ([dict valueForKey:@"msgStatusEvent"] != nil){
                NSString *strMsgStatus = [self getSeenMsgStatusIndexFromThreadChat:indexPath.row];
...
- (void)receiveMessageWithSenderId:(NSString *)senderId andDisplayName:(NSString *) displayName andtextMessage:(NSString *) textMessage msgType:(NSString*)msgType andDictionary:(NSDictionary*)dictResponse{ //andMsgStatus:(NSString *) msgStatus
    ...
    if ([msgType isEqualToString:@"image"]) {
        ...
        NSLog(@"ThreadChatViewController ->  receiveMessageWithSenderId -> image -> %%",textMessage);
        ...
        NSString*strURL = textMessage;
        NSLog(@"ThreadChatViewController -> receiveMessageWithSenderId -> gify -> %@",textMessage);
    ...
    /*else if ([msgType isEqualToString:@"video"]) {
     NSLog(@"ThreadChatViewController ->  receiveMessageWithSenderId -> video -> %@",textMessage);
     ...
     media:newMediaData];
        ...
        NSString*strURL = textMessage;
        NSLog(@"ThreadChatViewController ->  receiveMessageWithSenderId -> audio -> %@",textMessage);
        JSQAudioMediaItem *audioItem = [[JSQAudioMediaItem alloc] init];
        audioItem.delegate = self;
        if ([[NSFileManager defaultManager] fileExistsAtPath:strURL]) {
...
- (void)updateChatReplyThreadHistory {
    if (self.strThreadId != nil ) {
...
//         [self->_chatHistory insertObject:self.dictThreadMsgDetails atIndex:0];
        ...
        if ([[UserModel sharedInstance] getUserDetailsUsingKey:User_eRTCUserId] != nil) {
            ...
            if (self.dictUserDetails[App_User_ID] != nil && self.dictUserDetails[App_User_ID] != [NSNull null]) {
                ...
                if ([self.strReplyStatus isEqualToString:@"1"]){
                    if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"text"]){
                        [dictParam setObject:[self.dictThreadMsgDetails valueForKey:Message] forKey:Parent_Msg];
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"image"]){
                        NSString *mediaFile = [self.dictThreadMsgDetails valueForKey:@"mediaFileName"];
                        [dictParam setObject:mediaFile forKey:Parent_Msg];
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"video"]){
                        [dictParam setObject:[self.dictThreadMsgDetails valueForKey:@"mediaFileName"] forKey:Parent_Msg];
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"gify"]){
                        [dictParam setObject:[self.dictThreadMsgDetails valueForKey:@"mediaFileName"] forKey:Parent_Msg];
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:AudioFileName]){
                        [dictParam setObject:[self.dictThreadMsgDetails valueForKey:@"mediaFileName"] forKey:Parent_Msg];
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"location"]){
                        if (![Helper stringIsNilOrEmpty:[self.dictThreadMsgDetails valueForKeyPath:@"location.address"]]){
                            [dictParam setObject:[self.dictThreadMsgDetails valueForKeyPath:@"location.address"] forKey:Parent_Msg];
                        }else{
                            [dictParam setObject:@"" forKey:Parent_Msg];
                        }
                    }
                    else if ([[self.dictThreadMsgDetails valueForKey:@"msgType"] isEqualToString:@"contact"]){
                        if (![Helper stringIsNilOrEmpty:[self.dictThreadMsgDetails valueForKeyPath:@"contact.name"]]){
                            [dictParam setObject:[self.dictThreadMsgDetails valueForKeyPath:@"contact.name"] forKey:Parent_Msg];
                        }else{
                            [dictParam setObject:@"" forKey:Parent_Msg];
                        }
                    }
...
//              [dictParam setObject:@"text" forKey:MsgType];
                [dictParam setObject:[self.dictThreadMsgDetails valueForKey:ThreadID] forKey:ThreadID];
                [dictParam setObject:@"startThread" forKey:Start_Thread];
                [dictParam setObject:self.strReplyStatus forKey:ReplyMsgConfigStatus];
                [dictParam setObject:[self.dictThreadMsgDetails valueForKey:MsgUniqueId] forKey:ParentID];
                ...
                [[eRTCChatManager sharedChatInstance] sendTextMessageWithParam:[NSDictionary dictionaryWithDictionary:dictParam] andCompletion:^(id  _Nonnull json, NSString * _Nonnull errMsg) {
                    ...
                    if (dictResponse[@"success"] != nil) {
                        ...
                        if (success == true) {
                            ...
                            [self updateChatReplyThreadHistory];
                            [self.collectionView reloadData];
                            _aryMentioned = [NSMutableArray new];
                            if (_dictThreadMsgDetails[Follow_Message] != NULL && [_dictThreadMsgDetails[Follow_Message]  isEqual: @0]){
                            }
                        }else{
        ...
        if (self.strThreadId!= nil && dictMessage[ThreadID] != [NSNull null]) {
            if (![Helper stringIsNilOrEmpty:[self.dictThreadMsgDetails valueForKey:MsgUniqueId]]){
                if (![Helper stringIsNilOrEmpty:[dictMessage valueForKey:@"replyThreadFeatureData"]]){
                    ...
                    if ([[self.dictThreadMsgDetails valueForKey:MsgUniqueId] isEqual: [dictMessage valueForKeyPath:@"replyThreadFeatureData.baseMsgUniqueId"]]){
...
-(void)isMarkThreadMessageFavouriteWithIndexPath:(UIButton *)sender{
   ...
   // [self isMarkFavouriteWithIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
}
...
- (void)loadPreviousMessages:(NSMutableDictionary *)details completion:(LoadPreviouseMessageCompletion) completion {
 [[eRTCChatManager sharedChatInstance] loadPreviousChatHistoryWithThreadID:_dictThreadMsgDetails[ThreadID] parameters:details.copy
        ...
        //[[eRTCCoreDataManager sharedInstance] getUserReplyThreadChatHistoryWithThreadID:_dictThreadMsgDetails[ThreadID] andCompletionHandler:^(id ary, NSError *err) {
            NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:@"createdAt" ascending:YES];
            NSArray *sortedArray=[ary sortedArrayUsingDescriptors:@[sort]];
//            self->_chatHistory = [NSMutableArray arrayWithArray:sortedArray];
//            self->_message = [[NSMutableArray alloc] init];
...
-(void)refershControlAction {
    ...
    NSMutableDictionary *details = @{}.mutableCopy;
    details[@"pageSize"] = @20;
    details[@"currentMsgId"] =  _dictThreadMsgDetails[MsgUniqueId] ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([config[@"userMentionsChat"] boolValue] == true){
        ...
        if ((self->mentionsUser != nil && [self->mentionsUser length] > 0) && (self->mentionUserEmail != nil && [self->mentionUserEmail length] > 0) ) {
            ...
            if ([[[self.numbersArrayList objectAtIndex:indexPath.row] valueForKey:@"appUserId"] isEqualToString:@"channel"] || [[[self.numbersArrayList objectAtIndex:indexPath.row] valueForKey:@"appUserId"] isEqualToString:@"here"]){
                [dict setValue:@"generic" forKey:@"type"];
                [dict setValue:@"channel" forKey:@"value"];
            }else{
                [dict setValue:@"user" forKey:@"type"];
                [dict setValue:[[self.numbersArrayList objectAtIndex:indexPath.row] valueForKey:@"appUserId"] forKey:@"value"];
            }
...
- (void)deleteMessage:(NSString *)type {
    NSLog(@"Thread Single Delete %@ type", type);
    NSDictionary * dictMessage = [NSDictionary new];
...
-(void)updateMessageCellAtIndexPath:(NSIndexPath*)path message:(NSDictionary*)details{
    [[eRTCCoreDataManager sharedInstance] getUserReplyThreadChatHistoryWithThreadID:self.strThreadId withParentID:[self.dictThreadMsgDetails valueForKey:MsgUniqueId] andCompletionHandler:^(id ary, NSError *err) {
        ...
        NSLog(@"SingleChatViewController ->  getChatHistory -> getUserChatHistoryWithThreadID -> %@",ary);
       // self->_chatHistory = [NSMutableArray arrayWithArray:ary];
        NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:@"createdAt" ascending:YES];
        NSArray *sortedArray=[ary sortedArrayUsingDescriptors:@[sort]];
...
- (void)recentChatReactionDelegate:(int)tagId selectedIndexPath:(NSIndexPath * _Nullable)indexPath emojiCode:(NSString * _Nullable)message {
     if (tagId == 106) {
          ...
          //  [dictParam setObject:[dictMessage valueForKey:MsgUniqueId] forKey:ParentID];
...
/*- (void)recentChatReactionDelegate:(int)tagId selectedIndexPath:(NSIndexPath *)indexPath search:(NSString *)message {
    if (tagId == 106) {
        EmojisViewController *emojisVC = [[Helper mainStoryBoard] instantiateViewControllerWithIdentifier:@"EmojisViewController"];
        emojisVC.delegate = self;
        emojisVC.selectedIndexPath = indexPath;
